...
pr_ein8:- City = [_,_,_,_,_],
in_list(City,[sergey,_,riga]),
in_list(City,[boris,riga,penza]),
in_list(City,[victor,livov,moscow]),
in_list(City,[georgie,_,harkov]),
in_list(City,[_,penza,X]),
in_list(City,[leonid,X,_]),
in_list(City,[_,harkov,_]),
in_list(City,[_,moscow,_]),
in_list(City,[_,_,livov]),
not(in_list(City,[_,harkov,harkov])),
not(in_list(City,[_,penza,penza])),
not(in_list(City,[_,moscow,moscow])),
in_list(City,[Who,moscow,_]),
write(City), nl,
write(Who), write(" - student moscow").
